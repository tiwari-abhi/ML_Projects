...
def navigate(user='tiwari.abhi@husky.neu.edu',password='lk27u:Xw'):
    global web
    web = Browser()
        ...
        for f in abc:
            sample_df = pd.read_csv(f, sep="|",
                                    names=['fico', 'dt_first_pi', 'flag_fthb', 'dt_matr', 'cd_msa', "mi_pct",
                                           ...
                                           'servicer_name', 'flag_sc'], skipinitialspace=True)
...
def createPerformanceCombined(sample_perf_files):
    ...
    if "sample" in sample_perf_files:
        filename = "Sample Performance CombinedSummary.csv"

    abc = glob.glob(sample_perf_files)

    with open(filename, 'w', encoding='utf-8', newline="") as file:
        for f in abc:
            perf_df = pd.read_csv(f,sep='|', names= ['loan_seq', 'mth_per', 'current_upb', 'delq_sts', 'loan_age', 'mths_remng',
                                         ...
                                         'taxes_ins_costs', 'misc_costs', 'actual_loss', 'modcost','step_mod_flag','def_py_mod','eltv'],
                                          skipinitialspace=False,low_memory=False)
            ...
            summ_df = summ_df.join((perf_df['current_upb'].groupby(perf_df['loan_seq']).apply(get_current_upb).unstack()), on='loan_seq')
            summ_df = summ_df.join((perf_df['delq_sts'].groupby(perf_df['loan_seq']).apply(get_delq_sts).unstack()), on='loan_seq')
            summ_df = summ_df.join((perf_df['cd_zero_bal'].groupby(perf_df['loan_seq']).apply(get_cd_zero_bal).unstack()), on='loan_seq')
            summ_df = summ_df.join((perf_df['non_mi_recoveries'].groupby(perf_df['loan_seq']).apply(get_non_mi_recoveries).unstack()), on='loan_seq')
            summ_df = summ_df.join((perf_df['expenses'].groupby(perf_df['loan_seq']).apply(get_expenses).unstack()), on='loan_seq')
            summ_df = summ_df.join((perf_df['legal_costs'].groupby(perf_df['loan_seq']).apply(get_legal_costs).unstack()), on='loan_seq')
            summ_df = summ_df.join((perf_df['maint_pres_costs'].groupby(perf_df['loan_seq']).apply(get_maint_pres_costs).unstack()), on='loan_seq')
            summ_df = summ_df.join((perf_df['taxes_ins_costs'].groupby(perf_df['loan_seq']).apply(get_taxes_ins_costs).unstack()), on='loan_seq')
            summ_df = summ_df.join((perf_df['misc_costs'].groupby(perf_df['loan_seq']).apply(get_misc_costs).unstack()), on='loan_seq')
            summ_df = summ_df.join((perf_df['actual_loss'].groupby(perf_df['loan_seq']).apply(get_actual_loss).unstack()), on='loan_seq')
            summ_df = summ_df.join((perf_df['modcost'].groupby(perf_df['loan_seq']).apply(get_modcost).unstack()), on='loan_seq')
